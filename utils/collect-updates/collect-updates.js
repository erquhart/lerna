...
function collectUpdates(filteredPackages, packageGraph, execOpts, commandOptions) {
  ...
  if (forced.size) {
    ...
    const forceLabel = useConventionalGraduate ? "conventional-graduate" : "force-publish";
    log.warn(forceLabel, forced.has("*") ? "all packages" : Array.from(forced.values()).join("\n"));
  }
  ...
  // --force-publish or no tag
    ...
    const onInclude = name => log.verbose("updated", name);
    return collectPackages(packages, { onInclude });
  }
  ...
  const isCandidate = (node, name) => isForced(node, name) || needsBump(node) || hasDiff(node);
  const onInclude = name => log.verbose("updated", name);
  return collectPackages(packages, { isCandidate, onInclude });
}
